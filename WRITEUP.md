...
a. I assumed usage sessions are reported in whole minutes because the requirement charges one minute per minute. The API rejects non-whole-minute sessions, such as 30 seconds or 1 minute 59 seconds, instead of rounding them.
...
This application is a single-process SQLite app, so I am not claiming a distributed multi-process concurrency guarantee. Inside this app, each usage request is handled in a database transaction, and the second request sees the balance left by the first committed request.
...
So the ledger would effectively contain `-7` and then `-3`. If App B is processed first, the same thing happens with the app names reversed: the first request consumes 7, the second consumes only the remaining 3, and 4 minutes are rejected.
...
My route test initiates two overlapping usage requests with `Promise.all` and verifies that the combined covered minutes do not exceed the starting balance, both sessions are recorded, the final balance is non-negative, and the ledger sum still equals the child balance. Because the app and test use one SQLite-backed process, this remains a deterministic single-process concurrency test rather than a distributed database race test.
...
The deduction of the remaining 10 minutes is recorded as an UNDO_APPROVAL ledger entry. I also record the debt change separately using debt_change, while keeping it separate from amount. This means amount represents available-balance changes, while debt_change makes the debt movement auditable.
...
I would also add proper indexes on frequently queried fields such as child_id, task_id, and usage session IDs
...
I deliberately kept this as a small single-process assessment app. I did not build production-grade PostgreSQL deployment, a migration system, structured observability, authentication hardening beyond the token flow, or distributed concurrency tests across multiple app processes.

With one more week, I would move the schema to PostgreSQL with real migrations, add indexes around `child_id`, usage IDs, task IDs, and ledger references, and keep the balance and ledger writes inside database transactions. I would add integration tests that run multiple app processes against the same database so the overlapping usage case is tested under real database locking and isolation, not only inside one Node process. I would also add request logging and a few operational checks around failed usage batches and ledger invariant drift, because those are the places I would want fast visibility in production.
